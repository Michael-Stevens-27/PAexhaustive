...
tau <- runif(1, 1, 3)
  ...
  # DPM - fit sigma
  mean(m$sigma)
  hit_params <- geoParams(data = hit_data, sigma_mean = mean(m$sigma), sigma_squared_shape = 2, samples= 50000, chains = 200, burnin = 2000, priorMean_longitude = mean(hit_data$longitude), priorMean_latitude = mean(hit_data$latitude), guardRail = 0.05)
  hit_params$output$longitude_minMax <- master_params$output$longitude_minMax
  hit_params$output$latitude_minMax <- master_params$output$latitude_minMax
  m <- geoMCMC(data=hit_data, params= hit_params)

  ### PA ###
  Trap_Data$Trap_cap_density <- as.data.frame(Trap_Data$Trap_cap_density)
  Data_parameters <- Extract_Params(sim, Trap_Data$Trap_cap_density, PA_x_grid_cells = 50, PA_y_grid_cells = 50, Guard_Rail = 0.05, Trap_Radius = Trap_Data$detection_TR, n_sources = n_sources, n_cores = 1, n_offenders = n_offenders, Time = 1, Sd_x = Trap_Data$Sd, Sd_y = Trap_Data$Sd)
  Trap_Poisson_Params <- Trap_Po_Parameters(Data_parameters)
  Source_Probabilities <- Multisource_probs(Data_parameters, Trap_Poisson_Params)
  Michael_geoReportHitscores(Data_parameters, s, Source_Probabilities$Source_Both)
...
length(Source_Probabilities$LSource_Both[1,])
